...
def build_parser():
    ...
    p.add_argument(
        "--output-h5ad", default=None,
        help="If set, write the AnnData with predictions in .obs to this path.",
    )
    p.add_argument(
        "--clustering", choices=("ccc", "precomputed"), default="ccc",
    ...
    p.add_argument(
        "--output-format", choices=("name", "cl_id", "key"), default="name",
    ...
    p.add_argument("--n-neighbors-lvl2", type=int, default=10,
                   help="Level-2 nearest neighbors (ccc mode, default: 10).")
    p.add_argument("--n-neighbors-lvl3", type=int, default=5,
                   help="Level-3 nearest neighbors (ccc mode, default: 5).")
    p.add_argument("--n-smooth", type=int, default=1,
                   help="Smoothing iterations (ccc mode, default: 1).")
    p.add_argument("--ref-top-k", type=int, default=200,
                   help="Top consensus markers per cell type (default: 200).")
    p.add_argument("--graph-level", type=int, default=3, choices=(1, 2, 3),
                   help="Consensus level for the ccc kNN graph (default: 3).")
    p.add_argument("--include-cancer", action="store_true",
                   help="Add cancer key(s) as level-1 classes during fit.")
    p.add_argument(
        "--cancer-types", default=None,
    ...
    p.add_argument("--save-model", default=None,
                   help="Persist the fitted model to this .pkl path.")
...
def _parse_levels(levels_str):
    ...
    if not levels or any(lvl not in (1, 2, 3) for lvl in levels):
        raise SystemExit("error: --levels must be a comma-separated subset "
                         "of {1,2,3}.")
    return levels
...
def main(argv=None):
    ...
    cc.fit(adata, include_cancer=args.include_cancer,
           cancer_types=cancer_types, verbose=verbose)
    ...
    if args.output_h5ad:
        ...
        if verbose:
            print(f"Wrote annotated AnnData to {args.output_h5ad}",
                  file=sys.stderr)
