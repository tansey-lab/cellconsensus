...
def build_sub_nn(nn_indices, cell_idx, sub_n_neighbors):
    ...
    global_to_local = {int(g): l for l, g in enumerate(cell_idx)}
    sub_nn = np.zeros((len(cell_idx), sub_n_neighbors), dtype=int)
    for li, gi in enumerate(cell_idx):
        local = [global_to_local[int(nb)] for nb in nn_indices[gi]
...
def run_sublevel(Q, cell_labels, child_level, var_names, reduce_fn,
                 ref_top_k=200, skip_labels=("unassigned",), verbose=True):
    ...
    for parent in parents:
        siblings = sorted([k for k in child_consensus
        ...
        if verbose:
            print(f"  {parent}: {len(cell_idx)} cells, "
                  f"{len(siblings)} subtypes")
    ...
    if verbose:
        ...
        for mk in sorted(set(new_labels)):
            ...
            if count > 0:
                ...
                print(f"    {child_groups.get(mk, mk):<35} {count:>6} cells  "
                      f"(avg score: {avg:.4f})")
